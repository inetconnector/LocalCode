...
func blockedAvoidanceQuestion(task, question string) (bool, string) {
	...
	buildSystemMention := strings.Contains(q, "buildsystem") || strings.Contains(q, "build system")
	if buildSystemMention && (strings.Contains(q, "kompiliert werden soll") || strings.Contains(q, "gebaut werden soll") || strings.Contains(q, "build befehl") || strings.Contains(q, "build datei") || strings.Contains(q, "build command") || strings.Contains(q, "build file") || strings.Contains(q, "how should this be built") || strings.Contains(q, "how it should be built") || strings.Contains(q, "how the project should be built") || strings.Contains(q, "csproj angeben") || strings.Contains(q, "specify a csproj")) {
		return true, "Es gibt noch kein Buildsystem, weil dieses Projekt neu erstellt werden soll. Frage nicht danach. Wähle selbst ein Standard-Projektlayout passend zur in der Aufgabe genannten Sprache/Plattform (z.B. .NET: ein lauffähiges SDK-Style .csproj; Node: package.json; Python: pyproject.toml), lege es mit write_file an und implementiere den vollständigen angeforderten Umfang direkt."
	}
