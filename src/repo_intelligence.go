...
var repoIntelSymbolPatterns = []*regexp.Regexp{
	...
	regexp.MustCompile(`^\s*(?:export\s+)?(?:default\s+)?(?:interface|type|enum)\s+([A-Za-z_$][A-Za-z0-9_$]*)`),
	...
	regexp.MustCompile(`^\s*(?:export\s+)?(?:public|private|protected|internal|static|sealed|abstract|partial|final|open|data|suspend|override|virtual|async|unsafe|extern|new|readonly|ref|record|\s)+\s*(?:class|interface|record|enum|struct)\s+([A-Za-z_][A-Za-z0-9_]*)\b`),
	regexp.MustCompile(`^\s*(?:export\s+)?(?:public|private|protected|internal|static|final|open|suspend|override|async|virtual|abstract|extern|unsafe|\s)+\s*[A-Za-z_<>,?\[\].:]+\s+([A-Za-z_][A-Za-z0-9_]*)\s*\(`),
