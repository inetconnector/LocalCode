...
Der Startkontext des nativen Agenten folgt jetzt einer mehrstufigen Regelkette. LocalCode lädt globale LocalCode- und kompatible Codex-Anweisungen, projektnahe `AGENTS.override.md`/`AGENTS.md`-Dateien, kompatible Projektanweisungen wie `CLAUDE.md`, `GEMINI.md`, `QWEN.md`, `.agents.md`, `TEAM_GUIDE.md` und `.github/copilot-instructions.md`, `README.md` und `STATE.md`, legacy `.cursorrules`, verschachtelte `.cursor/rules` sowie Skills aus Projekt- und globalen Verzeichnissen wie `.codex/skills`, `.cursor/skills`, `.opencode/skills`, `skills`, der LocalCode-Konfiguration, `CODEX_HOME`, `.cursor/skills` und `.opencode/skills`. `alwaysApply`, `description`, `globs`, `activation`/`triggers`/`keywords`/`when`, `exclude_globs` und `priority` sowie Skill-Metadaten wie `permissions`, `allowed-tools`, `tools`, `scripts` und `commands` werden berücksichtigt; Frontmatter-Listen funktionieren als YAML-Liste, Inline-Array oder kommaseparierte Werte mit zitierten Kommas. Gleichnamige Skills werden deterministisch aufgelöst: Projekt-Skills gewinnen vor globalen Skills, innerhalb derselben Ebene entscheidet höhere Priorität. Skills mit Berechtigungen jenseits bekannter read-only Aliase wie `Read`, `Glob`, `Grep`, `skill_read`, `command_read` und MCP-List/Read-Aktionen oder mit deklarierten Skripten werden als `approval-required` indexiert und nicht automatisch als Arbeitsanweisung eingebettet. Zusätzlich kann das Modell mit `skill_list`, `skill_read`, `skill_list_resources` und `skill_read_resource` verfügbare Skills und deren Textressourcen progressiv nachladen; binäre oder als Projektdatei benötigte Skill-Ressourcen werden mit `skill_copy_resource` nach Genehmigung innerhalb der Projektgrenze kopiert. Deklarierte Skill-Skripte oder -Commands können mit `skill_run_script` nur exakt aus der Skill-Metadatenliste und nach eigener Genehmigung gestartet werden.

Wiederverwendbare Slash-/Projekt-Commands werden als Text-Templates aus `.localcode/commands`, kompatibel aus `.codex/commands`, `.opencode/commands`, `.cursor/commands` und global aus dem LocalCode-Konfigurationsordner geladen. Dateien enden auf `.md` oder `.txt`, der Dateiname wird zum Command-Namen, optionales Frontmatter `description` beschreibt den Command. Eine Eingabe wie `/review src/main.go` wird vor einem neuen Agentenlauf in die gespeicherte Anweisung expandiert; `{{args}}`, `{{project}}` und `{{cwd}}` werden ersetzt. Das Modell kann verfügbare Commands zusätzlich read-only mit `command_list` und `command_read(name)` inspizieren. Projekt-Commands überschreiben globale oder kompatible gleichnamige Commands deterministisch, führen aber selbst keine Shell-Befehle aus.
...
Wenn lokale Modelle trotz derselben Abschlussprüfungsbefunde weiter `finish` melden, eskaliert LocalCode Einzeldatei-Aufgaben über einen fokussierten inhaltsbezogenen Reparaturaufruf und schreibt die geforderte Datei anschließend durch die normale `write_file`-Grenze. Große Schreib-/Editierinhalte werden nach der Ausführung in der Modellhistorie kompaktiert; vollständige Werkzeugausgaben und Datei-Postconditions bleiben sichtbar. Explizite Datei-/Inhaltsprüfungen können durch passende Lese- oder Suchverifikation erfüllt werden; allgemeine Funktionstests verlangen weiterhin echte Syntax-, Build-, Test- oder Smoke-Prüfungen.

Für getrennte Exploration besitzt die native Werkzeugschleife `subagent_analyze`. Die Aktion kann jetzt einen isolierten modellgestützten **Explorer**, **Planner** oder **Reviewer** mit eigenem Kontext starten; ohne Rollenangabe wird Explorer verwendet. Jeder Child-Agent hat harte Modell-, Tool-, Zeit- und als Schätzung ausgewiesene Tokenbudgets und darf ausschließlich Projektbaum, Dateien, Textsuche und freigabefreies LSP lesen, bevor er mit einem strukturierten `AgentResult` endet. Schreib-/Lösch-/Move-Aktionen, Shell, Git, Netzwerk/Web, MCP, Installationen, Memory, Genehmigungen und rekursives Subagent-Spawning sind technisch nicht Teil seines Action-Schemas. Wenn kein Modell verfügbar ist, der Child-Lauf fehlschlägt oder ein Budget endet, fällt LocalCode auf den bestehenden deterministischen read-only Repository-Handoff zurück. Der obligatorische Reliability-Preflight vor Edits bleibt absichtlich deterministisch und verbraucht keine zusätzlichen Child-Modellaufrufe.
...
Bild-, Zeichen-, Diagramm- und Asset-Aufgaben müssen eine konkrete Artefaktdatei ändern, etwa SVG, HTML/Canvas, CSS oder eine Bilddatei. Eine Beschreibung oder ein SVG-Beispiel nur in einer README zählt nicht als fertiges Bild. Für lokale Icons, Diagramme und Vektorbilder stellt die native Werkzeugschleife `create_svg_asset` bereit: Das Modell liefert vollständiges SVG, LocalCode validiert XML-Struktur, Größenangabe und blockiert Skripte/Event-Handler, bevor die Datei geschrieben wird. Für lokale Raster- und Icon-Dateien stellt `create_image_asset` eine validierte Binärgrenze bereit: PNG, JPG/JPEG, GIF, WebP, ICO und BMP werden aus Data-URL/Base64 dekodiert, per Signatur und Dimensionen geprüft und erst danach geschrieben. `generate_image_asset` erzeugt neue PNG/JPG/JPEG/WebP/ICO-Dateien über einen lokal konfigurierten AUTOMATIC1111-/Forge-kompatiblen Loopback-Endpunkt, lehnt externe Provider-URLs ab und schreibt erst nach Decoding, Konvertierung und erneuter Bildvalidierung. `convert_image_asset` konvertiert vorhandene lokale PNG/JPG/JPEG/GIF- und PNG-basierte ICO-Dateien nach PNG, JPG/JPEG, WebP oder ICO, optional mit neuer Größe, und validiert das Ergebnis erneut. Zusätzlich rendert `render_asset` lokale SVG-, HTML- und Canvas-Quellen über vorhandenes Edge/Chrome in PNG, JPG/JPEG, WebP oder ICO und prüft die erzeugte Bilddatei auf Signatur und Dimensionen. Explizit genannte Bildpfade wie `assets/castle.svg` oder `preview.webp` werden als verpflichtende Projektdateien erkannt.
...
Der native Agent besitzt lokale, dauerhafte Erinnerungen. Mit `memory_remember`, `memory_list` und `memory_forget` kann er wichtige Projekt- oder Nutzerfakten speichern, im nächsten Agentenlauf wieder in den Kontext bekommen und per konkreter ID löschen. Zusätzlich erkennt LocalCode direkte Formulierungen wie „mache immer …, merke dir das“, „zeige gemerkt Liste“ oder „entferne aus der gemerkt Liste …“ deterministisch vor dem Modelllauf. „Immer“/„global“/„für alle Projekte“ wird global gespeichert und damit in jedem Projektkontext angewendet; projektbezogene Erinnerungen bleiben der Standard, wenn kein globaler Bezug genannt ist. Inhalte, die wie Passwörter, Tokens, private Schlüssel oder API-Keys aussehen, werden abgelehnt.
...
The native agent's startup context now follows a layered rule chain. LocalCode loads global LocalCode and compatible Codex instructions, project-local `AGENTS.override.md`/`AGENTS.md` files, compatible project instructions such as `CLAUDE.md`, `GEMINI.md`, `QWEN.md`, `.agents.md`, `TEAM_GUIDE.md`, and `.github/copilot-instructions.md`, `README.md` and `STATE.md`, legacy `.cursorrules`, nested `.cursor/rules`, and skills from project and global directories such as `.codex/skills`, `.cursor/skills`, `.opencode/skills`, `skills`, the LocalCode configuration, `CODEX_HOME`, `.cursor/skills`, and `.opencode/skills`. `alwaysApply`, `description`, `globs`, `activation`/`triggers`/`keywords`/`when`, `exclude_globs`, and `priority`, plus skill metadata such as `permissions`, `allowed-tools`, `tools`, `scripts`, and `commands` are considered; frontmatter lists work as YAML lists, inline arrays, or comma-separated values with quoted commas. Duplicate skill names are resolved deterministically: project skills win over global skills, and higher priority wins within the same tier. Skills with permissions beyond known read-only aliases such as `Read`, `Glob`, `Grep`, `skill_read`, `command_read`, and MCP list/read actions or with declared scripts are indexed as `approval-required` and are not automatically embedded as working instructions. The model can also progressively load available skills and their text resources through `skill_list`, `skill_read`, `skill_list_resources`, and `skill_read_resource`; binary or project-file skill resources can be copied into the project boundary with approval through `skill_copy_resource`. Declared skill scripts or commands can be started with `skill_run_script` only by exact metadata-list match and after a separate approval.

Reusable slash/project commands are loaded as text templates from `.localcode/commands`, compatibly from `.codex/commands`, `.opencode/commands`, `.cursor/commands`, and globally from the LocalCode configuration directory. Files use `.md` or `.txt`; the filename becomes the command name, and optional `description` frontmatter describes it. A prompt such as `/review src/main.go` is expanded before a new agent run into the stored instruction; `{{args}}`, `{{project}}`, and `{{cwd}}` are substituted. The model can also inspect available commands read-only through `command_list` and `command_read(name)`. Project commands deterministically override global or compatible commands with the same name, but commands themselves do not execute shell code.
...
For local models that keep trying to finish despite the same completion-guard findings, LocalCode now escalates single-file tasks through a focused content-only repair call and then writes the requested file through the normal `write_file` boundary. Large write/edit payloads are compacted in the model history after execution while the full tool result and file postconditions stay visible. Explicit file/content verification requests can be satisfied by a matching read/search verification; generic functional tests still require a real syntax, build, test, or smoke check.

For separate exploration, the native tool loop provides `subagent_analyze`. The action can now start an isolated model-backed **Explorer**, **Planner**, or **Reviewer** with its own context; Explorer is the default role. Every child has hard model-call, tool-call, time, and explicitly estimated token budgets and may only inspect the project tree, files, text search, and approval-free LSP before finishing with a structured `AgentResult`. Write/delete/move actions, shell, Git, network/web, MCP, installation, memory, approvals, and recursive subagent spawning are technically absent from its action schema. If the model is unavailable, the child fails, or a budget is exhausted, LocalCode falls back to the existing deterministic read-only repository handoff. The mandatory edit-reliability preflight intentionally remains deterministic and consumes no extra child-model calls.
...
Image, drawing, diagram, and asset tasks must change a concrete artifact file such as SVG, HTML/canvas, CSS, or an image file. A description or SVG example only inside a README does not count as a finished image. For local icons, diagrams, and vector images, the native tool loop provides `create_svg_asset`: the model supplies complete SVG, and LocalCode validates XML structure, size metadata, and blocks scripts/event handlers before writing the file. For local raster and icon files, `create_image_asset` provides a validated binary boundary: PNG, JPG/JPEG, GIF, WebP, ICO, and BMP are decoded from data URLs/Base64, checked by signature and dimensions, and only then written. `generate_image_asset` creates new PNG/JPG/JPEG/WebP/ICO files through a locally configured AUTOMATIC1111/Forge-compatible loopback endpoint, rejects external provider URLs, and writes only after decoding, conversion, and renewed image validation. `convert_image_asset` converts existing local PNG/JPG/JPEG/GIF and PNG-backed ICO files into PNG, JPG/JPEG, WebP, or ICO, optionally with new dimensions, and validates the result again. In addition, `render_asset` renders local SVG, HTML, and canvas sources through an installed Edge/Chrome browser into PNG, JPG/JPEG, WebP, or ICO and checks the generated image file for signature and dimensions. Explicit image paths such as `assets/castle.svg` or `preview.webp` are recognized as required project files.
...
The native agent has local durable memories. Through `memory_remember`, `memory_list`, and `memory_forget`, it can store important project or user facts, receive them again in later agent context, and delete them by concrete ID. LocalCode also detects direct prompts such as "always ..., remember that", "show memory list", or "remove ... from memories" deterministically before the model run. "Always", "global", and "all projects" are stored globally and therefore applied in every project context; project-scoped memories remain the default when no global scope is stated. Content that looks like passwords, tokens, private keys, or API keys is rejected.
