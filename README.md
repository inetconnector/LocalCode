...
On Windows, LocalCode opens maximized, including setup, normal/fast startup and reopening from the tray. The title bar and Windows taskbar remain available. Manual resizing remains possible after startup; `/tray` still starts in the background.

Unter Windows öffnet LocalCode maximiert: bei Einrichtung, normalem/Schnellstart und erneutem Öffnen aus dem Tray. Titelleiste und Windows-Taskleiste bleiben verfügbar. Nach dem Start lässt sich das Fenster weiterhin manuell verkleinern; `/tray` startet weiterhin im Hintergrund.

## Windows development checks / Windows-Entwicklungsprüfungen

Run `go fmt ./...`, `go vet ./...`, and `go test -race -count=1 ./...` from `src`. Windows race tests require CGO and a MinGW-w64 C compiler (runtime version 8 or newer); [w64devkit](https://github.com/skeeto/w64devkit) provides a portable toolchain. Add its `bin` directory to PATH, open a new terminal and set `$env:CGO_ENABLED='1'` before the race test. Check `gcc --version` and `gcc --print-file-name=libsynchronization.a`. The normal application build remains independent of CGO. Desktop text entry requires both a window title and a nonblank control name; invalid targets are rejected before desktop interaction.

`go fmt ./...`, `go vet ./...` und `go test -race -count=1 ./...` im Ordner `src` ausführen. Windows-Race-Tests benötigen CGO und einen MinGW-w64-C-Compiler (Runtime ab Version 8); [w64devkit](https://github.com/skeeto/w64devkit) stellt eine portable Toolchain bereit. Dessen `bin`-Ordner zum PATH hinzufügen, ein neues Terminal öffnen und vor dem Race-Test `$env:CGO_ENABLED='1'` setzen. Mit `gcc --version` und `gcc --print-file-name=libsynchronization.a` prüfen. Der normale Anwendungsbuild bleibt unabhängig von CGO. Desktop-Texteingaben benötigen Fenstertitel und einen nichtleeren Steuerelementnamen; ungültige Ziele werden vor jedem Desktop-Zugriff abgewiesen.

## IDE extension / IDE-Erweiterung

Release downloads: [v6.9.3](https://github.com/inetconnector/LocalCode/releases/tag/v6.9.3) includes Windows, Android and VSIX packages. Registry listings remain pending. The Android wrapper currently identifies itself as 1.0; use the release asset/checksum to identify its build. Newer local builds and verification limits are recorded in [STATE.md](STATE.md).

Release-Downloads: [v6.9.3](https://github.com/inetconnector/LocalCode/releases/tag/v6.9.3) enthält Windows-, Android- und VSIX-Pakete. Registry-Einträge sind noch offen. Die Android-Hülle meldet derzeit Version 1.0; maßgeblich sind Release-Datei und Prüfsumme. Neuere lokale Builds und Prüfgrenzen stehen in [STATE.md](STATE.md).

**English:** LocalCode now includes a VS Code / Antigravity IDE extension with a sidebar and editor chat, task history, live events, model selection, explicit editor context, approvals and native Git review. Priority follow-up prompts can steer an active run in the IDE and Desktop: stale inference is interrupted, started tools finish safely, and newer instructions are considered before the next action. See [installation, requirements, verified limits and development](extensions/localcode/README.md). The VSIX and matching backend are available in release v6.9.3; registry publication status is tracked in `STATE.md`. No Claude Code/Antigravity model-quality or full-service parity is claimed.

**Deutsch:** LocalCode enthält jetzt eine VS-Code-/Antigravity-IDE-Erweiterung mit Seitenleiste und Editorchat, Aufgabenverlauf, Live-Ereignissen, Modellwahl, ausdrücklich angehängtem Editor-Kontext, Genehmigungen und nativem Git-Review. Vorrangige Folgeprompts steuern laufende Aufgaben in IDE und Desktop: Veraltete Inferenz wird unterbrochen, gestartete Werkzeuge werden sicher beendet und neue Hinweise vor dem nächsten Schritt berücksichtigt. [Installation, Voraussetzungen, geprüfte Grenzen und Entwicklung](extensions/localcode/README.md). VSIX und passendes Backend sind im Release v6.9.3 verfügbar; der Stand der Registry-Veröffentlichung steht in `STATE.md`. Gleiche Modellqualität oder vollständige Dienstparität zu Claude Code/Antigravity wird nicht behauptet.

<p align="center">
  <strong>Language / Sprache:</strong>
  <a href="#english">🇬🇧 English (Default)</a> &nbsp;|&nbsp;
  <a href="#deutsch">🇩🇪 Deutsch</a> &nbsp;|&nbsp;
  <a href="LocalCode-Demo/">🎮 Live Demo: Pac-Man Arcade</a> &nbsp;|&nbsp;
  <a href="https://github.com/inetconnector/LocalCode/releases">📦 Releases & Downloads</a>
</p>
...
👉 **[Explore the Live Pac-Man Arcade Demo & Full Prompt Logs in LocalCode-Demo/](LocalCode-Demo/)**
...
<p align="right"><em><a href="#deutsch">Switch to Deutsch 🇩🇪</a></em></p>
...
<p align="right"><em><a href="#english">Zu Englisch wechseln 🇬🇧</a></em></p>
