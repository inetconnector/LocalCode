...
Remote-Aufrufe für Status, Projekte, Aufgaben, Chat, Stop und Genehmigungen benötigen das Gerätetoken ausschließlich per `X-LocalCode-Remote-Token` oder Bearer-Header. Für EventSource fordert die authentifizierte Remote-App zuerst ein kurzlebiges, einmal verwendbares Stream-Ticket mit 45 Sekunden TTL an; das langlebige Gerätetoken wird nicht in die SSE-URL geschrieben. Die Remote-App erweitert keine Genehmigungen, Sandboxgrenzen, Werkzeugrechte oder Projektgrenzen. Sie ruft dieselben Backendfunktionen wie die Desktop-Oberfläche auf; Datei-Anhänge laufen durch dieselbe Größen- und Typvalidierung. Cross-Origin-POSTs werden über Origin- und Fetch-Site-Prüfung abgelehnt. Bei bewusst aktivierter LAN-Bindung verwendet der Remote-Server TLS mit lokal erzeugtem Zertifikat und Fingerprint-Pinning in der Android-Hülle; Windows-Firewall- und Netzwerksegment-Regeln bleiben zusätzliche Betriebssystem-/Netzwerkaufgaben. Die Android-JavaScript-Brücke ist bewusst schmal: Sie startet nur den nativen File-Chooser beziehungsweise Androids Speech-Recognizer und gibt ausgewählte Dateien oder erkannten Text an die ohnehin geladene Remote-Web-App zurück.
...
Hintergrundbefehle starten unter Windows ohne sichtbare Konsolenfenster. Das gilt auch für nicht-interaktive Start- und Remote-Firewall-Helfer; nur bewusste interaktive Logins beziehungsweise Windows-UAC-Entscheidungen bleiben sichtbar. Diese Kontrollen sind Anwendungsschutz und keine identische Betriebssystem- oder Virtualisierungssandbox der proprietären Codex-Infrastruktur.
...
Remote calls for status, projects, tasks, chat, stop, and approvals require the device token only through `X-LocalCode-Remote-Token` or a Bearer header. For EventSource, the authenticated Remote app first requests a short-lived, single-use stream ticket with a 45-second TTL; the long-lived device token is not placed in the SSE URL. The Remote app does not extend approvals, sandbox boundaries, tool rights, or project boundaries. It calls the same backend functions as the desktop UI; file attachments pass through the same size and type validation. Cross-origin POSTs are rejected through Origin and Fetch-Site checks. When LAN binding is deliberately enabled, the Remote server uses TLS with a locally generated certificate and fingerprint pinning in the Android shell; Windows Firewall and network-segment controls remain additional operating-system and network responsibilities. The Android JavaScript bridge is intentionally narrow: it only starts the native file chooser or Android speech recognizer and returns selected files or recognized text to the already loaded Remote web app.
...
Background commands start without visible console windows on Windows. This also covers non-interactive startup and Remote firewall helpers; only deliberate interactive logins or Windows UAC decisions remain visible. These controls are application-level protections, not an identical operating-system or virtualization sandbox to proprietary Codex infrastructure.
...
## Native Agent Scheduler security boundary / Sicherheitsgrenze

Scheduler resource admission is not permission admission. A queued task is admitted only when its role is one of the current executable read-only Native roles and its actual `Capabilities` already contain the role's required grants. `RequestedCapabilities` stays inert, and a dynamic Planner label still does not become executable merely because a resource slot is available.

The first scheduler slice owns only queue entries, resource counters, exact leases, child contexts, cancellation, and budget/resource snapshots. It defaults local model inference to one active slot and keeps cheap read/CPU, build, and exclusive integration classes separately bounded. Cancelling a task or mission releases scheduler-owned resources and contexts, but it does not replay, approve, or perform mutations. The scheduler does not create worktrees, widen Mobile permissions, bypass approvals, alter SHA/file preconditions, or start mutation-capable Builders.

Deutsch: Resource-Admission vergibt keine Werkzeugrechte. Der Scheduler begrenzt und beobachtet nur Queue, Slots, Leases, Kontexte und Budgets; `RequestedCapabilities`, dynamische Rollen, Sandbox-, Approval-, Mobile- und Atomic-Write-Grenzen bleiben unverändert.
