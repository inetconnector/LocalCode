...
- Nicht abgeschlossen am 2026-08-13 für diesen Schritt: unisoliertes `go test -count=1 ./...` scheiterte in `TestCodingEngineActionsInstallDecisionsAndUndo` und `TestCodingEngineServerValidationAndConflicts`, weil die lokale Benutzerumgebung ein echtes `claude.exe` als installiert, aber nicht authentifiziert erkennt und dadurch andere Engine-Setup-Pfade nimmt; zusätzlich startete ein Modell-Pull für `qwen2.5-coder:14b`. `go test -race -count=1 ./...` bleibt blockiert: ohne CGO `go: -race requires cgo`, mit `CGO_ENABLED=1` `cgo: C compiler "gcc" not found`. Das Playwright-UI-Skript bleibt blockiert, weil das Python-Modul `playwright` nicht installiert ist.
...
- [x] Lokale/globale Regelkette und relevante Skills: `AGENTS.override.md`/`AGENTS.md`, Codex-kompatible globale Anweisungen, Cursor-Regeln, README/STATE/Fallback-Dateien, Projekt- und globale Skill-Verzeichnisse, einfache `alwaysApply`-/`globs`-Relevanz und `skill_list`/`skill_read`.
...
- [ ] Vollständige Skills-/Regel-Parität: Skill- und Rule-Permissions, Skill-Unterressourcen/Assets/Skripte, komplexere Frontmatter-Varianten, skill-spezifische Aktivierungsbedingungen und Konfliktauflösung vollständig unterstützen. Quellen: Codex-`AGENTS.md`/Memories/Skills/MCP/Subagents, Cursor Rules/Skills, OpenCode Skills/Agents/Permissions.
...
- Rules/skills context parity step: the native agent now builds a bounded instruction chain from global LocalCode/Codex instructions, project `AGENTS.override.md`/`AGENTS.md`, fallback files such as `CLAUDE.md`, `README.md`, and `STATE.md`, relevant `.cursor/rules`, and skills from project and global directories (`.codex/skills`, `.cursor/skills`, `.opencode/skills`, `skills`, LocalCode config, `CODEX_HOME`, user `.cursor/skills`, user `.opencode/skills`). Rules and skills consider `alwaysApply`, `description`, and simple `globs`; `skill_list` and `skill_read` provide progressive read-only loading.
...
- Not completed on 2026-08-13 for this step: unisolated `go test -count=1 ./...` failed in `TestCodingEngineActionsInstallDecisionsAndUndo` and `TestCodingEngineServerValidationAndConflicts` because the local user environment detects a real `claude.exe` as installed but unauthenticated and therefore takes different engine setup paths; it also started a model pull for `qwen2.5-coder:14b`. `go test -race -count=1 ./...` remains blocked: without CGO `go: -race requires cgo`, with `CGO_ENABLED=1` `cgo: C compiler "gcc" not found`. The Playwright UI script remains blocked because the Python `playwright` module is not installed.
...
- [x] Local/global rule chain and relevant skills: `AGENTS.override.md`/`AGENTS.md`, Codex-compatible global instructions, Cursor rules, README/STATE/fallback files, project and global skill directories, simple `alwaysApply`/`globs` relevance, and `skill_list`/`skill_read`.
...
- [ ] Full skills/rules parity: fully support skill and rule permissions, skill subresources/assets/scripts, richer frontmatter variants, skill-specific activation conditions, and conflict resolution. Sources: Codex `AGENTS.md`/memories/skills/MCP/subagents, Cursor rules/skills, OpenCode skills/agents/permissions.
